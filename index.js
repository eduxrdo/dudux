		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD EDUARDO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Eduardo? Este é um recurso especial para o Eduardo ❌',
					ownerB: '[❗] Eduardo? Este é um recurso especial para o Eduardo ❌',
			...
			const ownerNumber = ["558588224016@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558588224016@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558588224016@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558588224016@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558588224016@s.whatsapp.net","558589944973@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('Pq tu ta falando isso porra, tu é adm seu idiota')
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Nesse grupo, não gostamos de racistas, que isso sirva de exemplo 🚶")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("2 segundos")
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("5 segundo KKKKKKKK tchau escroto 😔🤙")
		}, 0)
	}
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("pq vc é preto")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('Pq tu ta falando isso porra, tu é adm seu idiota')
		client.updatePresence(from, Presence.composing)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS DUDU BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Eduardo ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
